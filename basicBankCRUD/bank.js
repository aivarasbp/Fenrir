...
const generateUniqueId = () => {
  return "LT" + Math.random().toString(36).substr(2, 9); // Generate a random string
};
  ...
  accounts.forEach((account) => {
    accountsHtml += `
      <div class="accountModal">
        ...
        <div class="accBalance">Balance: ${account.balance}</div>
        <div class="accBtn">
        ...
        <div><a href="http://127.0.0.1:5500/bankas01/account.html">Open Account</a></div>
      </div>
    `;
  });
    ...
    button.addEventListener("click", (e) => {
      ...
      if (account) {
        if (account.balance === 0) {
          ...
          alert(
            "You cannot delete an account with a balance less than zero.",
          );
        }
      } else {
